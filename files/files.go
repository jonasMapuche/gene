package files

import (
	"io/ioutil"
	"os"
)

const directory string = "output/"
const File string = "gene.go"
const file_crypto string = "crypto.go"
const file_tree string = "tree.go"
const file_data string = "data.go"
const file_file string = "file.go"
const file_input string = "input.go"
const file_router string = "router.go"

func Body() {
	tree()
	crypto()
	gene()
	data()
	file()
	input()
	output()
	router()
}

func crypto() {
	var home string = directory + "crypto"
	var err_dir_crypto = /*error*/ os.Mkdir(home, 0777)
	if err_dir_crypto != nil {
		panic(err_dir_crypto)
	}

	var expression string = "package crypto\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func Cast() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"

	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte*/ []byte(expression)
	var err_crypto = /*error=*/ ioutil.WriteFile(home+"/"+file_crypto, text, 0644)
	if err_crypto != nil {
		panic(err_crypto)
	}
}

func tree() {
	var home string = directory + "trees"
	var err_dir_tree = /*error=*/ os.Mkdir(home, 0777)
	if err_dir_tree != nil {
		panic(err_dir_tree)
	}

	var expression string = "package trees\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func Add() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"

	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte=*/ []byte(expression)
	var err_tree = /*error=*/ ioutil.WriteFile(home+"/"+file_tree, text, 0644)
	if err_tree != nil {
		panic(err_tree)
	}
}

func gene() {
	var expression string = "package main\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func main() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"
	expression = expression + "fmt.Println(\"| Start...\")\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"
	expression = expression + "fmt.Println(\"| Data...\")\n"
	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "fmt.Println(\"| Tree...\")\n"
	expression = expression + "var article Notice\n"
	expression = expression + "article.Spawn = \"raiz\"\n"
	expression = expression + "article.Date = time.Date(2023, 1, 17, 0, 0, 0, 0, time.Local)\n"
	expression = expression + "article.Quantity = 4\n"
	expression = expression + "var tree *trees.Tree = trees.Add(trees.Notice(article))\n"
	expression = expression + "fmt.Println(\"| Tree: \", tree.String())\n"
	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte=*/ []byte(expression)
	var err_gene = /*error=*/ ioutil.WriteFile(directory+File, text, 0644)
	if err_gene != nil {
		panic(err_gene)
	}

}

func data() {
	var home string = directory + "data"
	var err_dir_data = /*error=*/ os.Mkdir(home, 0777)
	if err_dir_data != nil {
		panic(err_dir_data)
	}

	var expression string = "package data\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func Init() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"

	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte*/ []byte(expression)
	var err_data = /*error=*/ ioutil.WriteFile(home+"/"+file_data, text, 0644)
	if err_data != nil {
		panic(err_data)
	}
}

func file() {
	var home string = directory + "files"
	var err_dir_file = /*error=*/ os.Mkdir(home, 0777)
	if err_dir_file != nil {
		panic(err_dir_file)
	}

	var expression string = "package files\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func Body() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"

	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte*/ []byte(expression)
	var err_file = /*error=*/ ioutil.WriteFile(home+"/"+file_file, text, 0644)
	if err_file != nil {
		panic(err_file)
	}
}

func input() {
	var home string = directory + "input"
	var err_dir_input = /*error=*/ os.Mkdir(home, 0777)
	if err_dir_input != nil {
		panic(err_dir_input)
	}

	var expression string = "package input\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func Get() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"

	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte*/ []byte(expression)
	var err_input = /*error=*/ ioutil.WriteFile(home+"/"+file_input, text, 0644)
	if err_input != nil {
		panic(err_input)
	}
}

func output() {
	var home string = directory + "output"
	var err_dir_output = os.Mkdir(home, 0777)
	if err_dir_output != nil {
		panic(err_dir_output)
	}
}

func router() {
	var home string = directory + "router"
	var err_dir_router = os.Mkdir(home, 0777)
	if err_dir_router != nil {
		panic(err_dir_router)
	}

	var expression string = "package router\n\nimport (\n	\"fmt\"\n)\n\n"
	expression = expression + "func Controller() {\n"
	expression = expression + "fmt.Println(\" -------------------------------------------------\")\n"

	expression = expression + "fmt.Println(\"|-------------------------------------------------\")\n"
	expression = expression + "}"

	var text = /*[]byte*/ []byte(expression)
	var err_router = /*error=*/ ioutil.WriteFile(home+"/"+file_router, text, 0644)
	if err_router != nil {
		panic(err_router)
	}
}
